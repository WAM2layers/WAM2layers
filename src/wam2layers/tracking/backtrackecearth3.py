...
def read_data_at_time(t):
    """Get a single time slice from input data at time t. In order to suit the cmip6 daily data, the time should in T12:00:00"""
    ds = read_data_at_date(t)
    return ds.sel(time=t.to_pydatetime()+ timedelta(hours=12), drop=True)
