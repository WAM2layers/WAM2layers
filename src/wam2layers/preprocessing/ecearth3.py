...
from wam2layers.preprocessing.shared import (accumulation_to_flux,insert_level,
                                             calculate_humidity,
...
def load_data(variable, date, config):
    ...
    if variable in ["ua", "va", "hus"]: #wind speed and specific humidity
    ...
    if variable in ["ua", "va", "hus"] and isinstance(config.levels, list):
...
def preprocess_precip_and_evap(date, config):
    ...
    # evap = load_data("hfls", date, config) # evaporation(in m) 
    # # Note, the conversion from latent heat flux to evaporation was done already in separate script
    # precip = load_data("pr", date, config)  # total precipitation
    # precip = precip*3600*24/1000 # in kg m-2 s-1,and convert to m
    ...
    ########################## Modified in May.8, successed.####################################
    # Clip should be in a sperate script.
    # Note, the Unit of hfls from CMIP6 model is W/m2, pr is in kg/m2/s,so should convert to m 
    # All incoming units are accumulations (in m) since previous time step
    evap = load_data("hfls", date, config) # evaporation(in m) 
    evap = evap/28.356/1000  #convert W/m2 to m
...
def get_dp_modellevels(sp, levels):
    ...
    a = xr.DataArray(a, coords={"level": np.insert(levels, 0, -1)}) #levels, 0, -1
...
def get_dp_pressurelevels(q, u, v, ps, qs, us, vs):
    ...
    u = insert_level(u, interpolate(p_boundary, p, u, axis=1), 150000)
    v = insert_level(v, interpolate(p_boundary, p, v, axis=1), 150000)
    q = insert_level(q, interpolate(p_boundary, p, q, axis=1), 150000)
...
def prep_experiment(config_file):
    ...
    for date in get_input_dates(config):
        print(date)
        #date = f"{date1.to_pydatetime().year}"+ '-' f"{date1.to_pydatetime().month}"

        # 4d fields
        levels = config.levels
        q = load_data("hus", date, config)  # in kg kg-1
        u = load_data("ua", date, config)  # in m/s
        v = load_data("va", date, config)  # in m/s
        sp = load_data("psl", date, config)  # in Pa
        ...
        if config.level_type == "pressure_levels":
            d2m = load_data("tdps", date, config)  # Dew point in K
            #d2m = d2m[:,0,:,:] ## for IPSL, the tdps is a 4D dataset, and select the surface one
            q2m = calculate_humidity(d2m, sp)  # kg kg-1
            u10 = load_data("uas", date, config)  # in m/s
            v10 = load_data("vas", date, config)  # in m/s
