...
if int(calendar.isleap(years[-1])) == 0: # no leap year
    datelist = get_times_daily(dt.date(years[0],months[0],1), dt.date(years[-1],months[-1], months_length_nonleap[months[-1]-1]))[::-1]
else: # leap
    datelist = get_times_daily(dt.date(years[0],months[0],1), dt.date(years[-1],months[-1], months_length_leap[months[-1]-1]))[::-1] #[::-1] to run the datelist the other way around
...
def data_path_ea(year, month, day):
    save_empty_arrays_track = os.path.join(sub_interdata_folder, str(year) + '-' + str(month).zfill(2) + '-' + str(day).zfill(2) + 'Sa_track')
    save_empty_arrays_time = os.path.join(sub_interdata_folder, str(year) + '-' +str(month).zfill(2) + '-' + str(day).zfill(2) + 'Sa_time')
...
def data_path(previous_data_to_load,yearnumber,month,a):
    ...
    load_fluxes_and_storages = os.path.join(interdata_folder, str(yearnumber) + '-' + str(month).zfill(2) + '-' + str(a).zfill(2) + 'fluxes_storages.mat')
    ...
    save_path_track = os.path.join(sub_interdata_folder, str(yearnumber) + '-' + str(month).zfill(2) + '-' + str(a).zfill(2) + 'Sa_track')
    save_path_time = os.path.join(sub_interdata_folder, str(yearnumber) + '-' + str(month).zfill(2) + '-' + str(a).zfill(2) + 'Sa_time')
...
def get_Sa_track_backward(latitude,longitude,count_time,divt,Kvf,Region,Fa_E_top,Fa_N_top,Fa_E_down,Fa_N_down,
    ...
    for t in np.arange(np.int(count_time*divt),0,-1):
        ...
        Sa_track_after_Fa_P_E_down[0,1:-1,1:-1] = (Sa_track_after_Fa_down[0,1:-1,1:-1]
        + P_region[t-1,1:-1,1:-1] * (W_down[t,1:-1,1:-1] / W[t,1:-1,1:-1])
        - E[t-1,1:-1,1:-1] * (Sa_track_down[t,1:-1,1:-1] / W_down[t,1:-1,1:-1]))

        # top: add precipitation
        Sa_track_after_Fa_P_E_top[0,1:-1,1:-1] = (Sa_track_after_Fa_top[0,1:-1,1:-1]
        ...
        Sa_track_down[t-1,1:-1,1:-1] = Sa_track_after_Fa_P_E_down[0,1:-1,1:-1] - down_to_top[t-1,1:-1,1:-1] + top_to_down[t-1,1:-1,1:-1]
        Sa_track_top[t-1,1:-1,1:-1] = Sa_track_after_Fa_P_E_top[0,1:-1,1:-1] - top_to_down[t-1,1:-1,1:-1] + down_to_top[t-1,1:-1,1:-1]

    return Sa_track_top,Sa_track_down,north_loss,south_loss,east_loss,west_loss,down_to_top,top_to_down,water_lost
...
for date in datelist[1:]:
    ...
    if timetracking == 0: # I use timetracking = 0
        Sa_track_top,Sa_track_down,north_loss,south_loss,east_loss,west_loss,down_to_top,top_to_down,water_lost = get_Sa_track_backward(latitude,longitude,count_time,divt,Kvf,Region,Fa_E_top,Fa_N_top,Fa_E_down,Fa_N_down,
                                       Fa_Vert,E,P,W_top,W_down,Sa_track_top_last,Sa_track_down_last)
#    elif timetracking == 1:
...
#                                        timestep,Kvf,Region,Fa_E_top,Fa_N_top,Fa_E_down,Fa_N_down,Fa_Vert,E,P,W_top,W_down,Sa_track_top_last,Sa_track_down_last,Sa_time_top_last,Sa_time_down_last)
    ...
    north_loss_per_day=north_loss_per_day, south_loss_per_day= south_loss_per_day, east_loss_per_day=east_loss_per_day, west_loss_per_day= west_loss_per_day, water_lost_per_day=water_lost_per_day)
